...
struct Monkey {
    ...
    Operation operation;
    Test test;
    ull inspected_items = 0;
};

vector<Monkey> Monkeys;
// Modulo arithmetics being funny
ull crazy_div_check = 1;
        ...
        for (int m_i = 0; m_i < Monkeys.size(); m_i++) {
            ...
            for (int m_i_i = 0; m_i_i < m_i_n; m_i_i++) {
                ...
                if (Monkeys[m_i].operation.operation == Op::Addition) {
                    Monkeys[m_i].items[m_i_i] = 
                        (Monkeys[m_i].operation.second_item+Monkeys[m_i].items[m_i_i])%crazy_div_check;
                } else if (Monkeys[m_i].operation.operation == Op::Multiplication) {
                    Monkeys[m_i].items[m_i_i] = 
                        (Monkeys[m_i].items[m_i_i]*Monkeys[m_i].operation.second_item)%crazy_div_check;
                } else {
                    Monkeys[m_i].items[m_i_i] = 
                        (Monkeys[m_i].items[m_i_i] * Monkeys[m_i].items[m_i_i])%crazy_div_check;
                }
...
int main(int argc, char** argv) {
    if (argc > 1 || DEBUG_MODE) {
        ...
        while (getline(input_file, line)) {
            if (line.empty()) {
                ...
                if (line[0] == 'M') {
                    ...
                    int div_check = stoi(line.substr(21, line.size()-21));
                    Monkeys[cur_monkey_id].test.div_check = div_check;
                    crazy_div_check *= div_check;
                } else if (line[4] == 'I') {
                    if (line[7] == 't') {
        ...
        stuff();
        multiset<ull, greater<ull>> inspected_items_count;
